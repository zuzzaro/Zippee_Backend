// zippee-backend - File:src/routes/partners.js - v13
...
// Endpoint per creare un nuovo viaggio (per partner)
router.post('/trips', authenticateToken,  async (req, res) => {
    try {
        const { numPersone, tipoTratta, mezzoTrasporto, cittaPartenza, cittaArrivo, dataPartenza } = req.body;
        ...
        const newTrip = await db.query(
            'INSERT INTO viaggi (partner_id, num_persone) VALUES ($1, $2) RETURNING id, partner_id, num_persone, created_at',
            [partnerId, numPersone]
        );

        // Recupero l'id del nuovo viaggio e creo le nuove tratte
          const newRoute = await db.query(
            'INSERT INTO tratte (viaggio_id, tipo_tratta, mezzo_trasporto, citta_partenza, citta_arrivo, data_partenza) VALUES ($1, $2, $3, $4, $5, $6) RETURNING id, viaggio_id, tipo_tratta, mezzo_trasporto, citta_partenza, citta_arrivo, data_partenza',
            [newTrip.rows[0].id, tipoTratta, mezzoTrasporto, cittaPartenza, cittaArrivo, dataPartenza]
        );

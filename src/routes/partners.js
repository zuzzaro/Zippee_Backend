// zippee-backend - File:src/routes/partners.js - v20
...
router.post('/:id/trips', authenticateToken, async (req, res) => {
    try {
        const { numPersone, return_date, eco_trip_code, date_departure } = req.body; // Usiamo return_date invece di date_of_stay
        ...
        if (!numPersone || !return_date || !eco_trip_code) {
            return res.status(400).json({ error: 'Dati mancanti: numPersone, return_date, eco_trip_code sono obbligatori' });
        }

        // Inserisci i dati nella tabella viaggi
        const newTrip = await db.query(
            `INSERT INTO viaggi (partner_id, num_persone, created_at, date_departure, return_date, eco_trip_code)
             VALUES ($1, $2, $3, $4, $5, $6) RETURNING *`,
            [partnerId, numPersone, new Date().toISOString(), date_departure || null, return_date, eco_trip_code]
        );
...
// Endpoint per ottenere i dati aggregati dei viaggi di un partner
router.get('/:id/aggregated-data', authenticateToken, async (req, res) => {
    try {
        const { id } = req.params;

        // Query per ottenere il numero totale di viaggi
        const totalTrips = await db.query(
            'SELECT COUNT(*) FROM viaggi WHERE partner_id = $1',
            [id]
        );

        // Query per ottenere il numero di viaggi con Carbon Cancelling Code
        const tripsWithCarbonCode = await db.query(
            'SELECT COUNT(*) FROM viaggi WHERE partner_id = $1 AND carbon_cancelling_code IS NOT NULL',
            [id]
        );

        // Query per ottenere il totale delle persone
        const totalPeople = await db.query(
            'SELECT SUM(num_persone) FROM viaggi WHERE partner_id = $1',
            [id]
        );

        res.status(200).json({
            totalTrips: totalTrips.rows[0].count,
            tripsWithCarbonCode: tripsWithCarbonCode.rows[0].count,
            totalPeople: totalPeople.rows[0].sum || 0, // Se non ci sono viaggi, restituisce 0
        });
    } catch (error) {
        console.error(error);
        res.status(500).json({ error: 'Errore durante il recupero dei dati aggregati.' });
    }
});

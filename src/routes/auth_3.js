// zippee-backend - File:src/routes/auth.js - v3
...
router.post('/login-partner', async (req, res) => {
     try {
       ...
       // Genera il token JWT
       const token = jwt.sign(
            { id: partnerUser.rows[0].id, role: partnerUser.rows[0].role, partner_id:partnerUser.rows[0].partner_id }, //payload modificato
